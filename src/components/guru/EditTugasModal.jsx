// src/components/guru/EditTugasModal.jsx
"use client";
...
const EditTugasModal = ({
  ...
  // Load tugas data saat modal dibuka
  useEffect(() => {
    if (tugas) {
      ...
      setFormData({
        ...
        mataPelajaran: tugas.mataPelajaran?._id || tugas.mataPelajaran || "",
        kelas: tugas.kelas?._id || tugas.kelas || "",
        deadline: formattedDeadline,
        semester: tugas.semester || "ganjil",
        tahunAjaran: tugas.tahunAjaran || "2024/2025",
      });
    }
  }, [tugas]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await onUpdate(tugas._id, formData);
    } catch (error) {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="flex items-center gap-3 pt-4">
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="flex-1 px-6 py-3 bg-gradient-to-r from-info to-info-dark text-white rounded-xl font-medium hover:shadow-lg transition-all disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {isSubmitting ? (
                <>
